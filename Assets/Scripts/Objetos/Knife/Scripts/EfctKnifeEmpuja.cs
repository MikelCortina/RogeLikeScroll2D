...
[CreateAssetMenu(menuName = "Effects/Spawn Two Projectiles Empuje Persistent", fileName = "SpawnTwoProjectilesEmpujeEffect")]
public class SpawnTwoProjectilesEmpujeEffect : ScriptableObject, IPersistentEffect
{
    [Header("Configuración del proyectil")]
    public GameObject projectilePrefab;           // Prefab que idealmente tiene ProjectileEmpuje, Rigidbody2D y Collider2D
    ...
    public float pushForce = 6f;                  // fuerza con la que empuja al enemigo (impulso)
    ...
    private ProjectileEmpuje[] projScripts = new ProjectileEmpuje[2];
    ...
    private void EnsureInstancesCreated()
    {
        for (int i = 0; i < 2; i++)
        {
            if (instances[i] == null)
            {
                ...
                instances[i].name = $"EmpujeProj_{i}";
                ...
                var pr = instances[i].GetComponent<ProjectileEmpuje>();
                if (pr == null) pr = instances[i].AddComponent<ProjectileEmpuje>();

                // Inicializamos el script (pasamos maxDistance y pushForce)
                pr.Initialize(this, i, returnSpeed, collectDistance, maxDistance, pushForce);
                projScripts[i] = pr;
                collected[i] = false;
    ...
    private IEnumerator MainLoop()
    {
        while (runtimeOwner != null)
        {
            ...
            Debug.Log("[SpawnTwoProjectilesEmpujeEffect] Ambos recogidos -> esperando spawnInterval para relanzar.");
    ...
    #region Callbacks desde ProjectileEmpuje

    // llamado por ProjectileEmpuje cuando un proyectil colisiona con algo y debe volver
    ...
    // llamado por ProjectileEmpuje cuando el proyectil llega al owner (recogido)
